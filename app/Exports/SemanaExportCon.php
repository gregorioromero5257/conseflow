...
class SemanaExportCon implements FromView, ShouldAutoSize, WithEvents
{
  protected $id;
    ...
    return [
      ...
      AfterSheet::class => function (AfterSheet $event) use ($styleArray) {
        $drawing = new \PhpOffice\PhpSpreadsheet\Worksheet\Drawing();
              ...
              // $border= new PhpOffice\PhpSpreadsheet\Style();
              // $border= getStyleArray('A1');
      $event->sheet->getStyle('C6')->applyFromArray($styleArray[1]);
      $event->sheet->getStyle('A8:C8')->applyFromArray($styleArray[2]);
      $event->sheet->getStyle('A10:I10')->applyFromArray($styleArray[0]);//se lllama el array de la posicion 0
      for ($i=11; $i < 100; $i++) {
        // code...
          $event->sheet->getStyle('A'.$i.':I'.$i)->applyFromArray($styleArray[1]);//se lllama el array de la posicion 0
      }
      },
    ];
  }
  ...
  public function view(): View
  {
    ...
    // ->select('P.*')
    // ->where('contratos.tipo_nomina_id','=','1')->distinct()->get();
    ...
    ->where('C.tipo_nomina_id','=','1')->where('nominas_movimientos.nomina_id','=',$this->id)->get();
    ...
    ->where('contratos.tipo_nomina_id','=','1')
    ->first();

    // $empleados = \App\


    return view('excel.semanaldos', compact('contratosp','contrat_to','nomina'));
  }
}
