...
use App\Exports\SemanaExport;
use App\Exports\SemanaExportFin;
use App\Exports\SemanaExportCon;
...
class NominaSemanalController extends Controller
{
  ...
  {
    $nomina = \App\Nomina::where('tipo_nomina','=','2')->orderBy('id','DESC')->get();
    return response()->json($nomina);
  }

  public function excel($id)
  {
    return Excel::download(new SemanaExport($id), 'ReporteSemanal.xlsx' );
  ...
  public function exceluno($id)
  {
    ...
    return Excel::download(new SemanaExportFin($id), 'ReporteSemanalAdminFinan.xlsx' );
    // return (new QuincenaExport)->download('invoices.xlsx');
  }
  ...
  public function exceldos($id)
  {
    ...
    return Excel::download(new SemanaExportCon($id), 'ReporteSemanalContabilidad.xlsx' );
    // return (new QuincenaExport)->download('invoices.xlsx');
  }

  public function nominaexcel($id)
  {
    return Excel::download(new NominaSExport($id), 'ReporteNominaSemanal.xlsx' );
  }
  ...
  public function proyecto($id)
  {
    ...
    ->select('P.*')
    ->where('C.tipo_nomina_id','=','1')->where('nominas_movimientos.nomina_id','=',$id)->distinct()->get();
  ...
  public function busqueda_por_proyecto($id){
    ...
    ->Leftjoin('nomina AS n','n.id','nm.nomina_id')
    ->leftjoin('empleados AS e','e.id','nm.empleado_id')
    ...
    foreach ($mov_nomi as $key => $value) {
      ...
      }elseif($value->empresa == 3) {
        $empleado_csct = $this->empleado_csct($value->empleado_id);

        $bancos = \App\DatosBancariosEmpleadoDBP::
        leftjoin('catalogo_bancos AS b','b.id','datos_bancarios_empleados.banco_id')
  ...
  public function store(Request $request)
  {
    try {
      ...
      foreach ($empleados as $key => $value) {
        ...
        }elseif ($request->empresa == 1) {//conserflow
        ...
        ->whereNotIn('ct.proyecto_id',[1,0,61,100,110,113])
        ...
        $checador_registros = DB::table('reg_checador AS rc')
        ->select('rc.fecha')
        ->where('rc.empleado_id',$value->id)
        ->whereBetween('rc.fecha',[$request->fecha_inicial,$request->fecha_final])
        ->groupBy('rc.fecha')
        ->orderBy('rc.fecha','ASC')
        ->get();

        if (count($checador_registros) > 0 && count($checador_registros) <= 6) {
          $asistencias = count($checador_registros) + 1;
        }elseif (count($checador_registros) >= 7) {
          $asistencias = count($checador_registros);
        }

        if ($asistencias == 0) {
          if (count($contro_tiempo_registros) > 0 && count($contro_tiempo_registros) <= 6) {
            $asistencias = count($contro_tiempo_registros) + 1;
          }elseif (count($contro_tiempo_registros) >= 7) {
            $asistencias = count($contro_tiempo_registros);
          }
        }
        //2 ,4
  ...
  public function editarnominasemanal(Request $request){
    ...
    $total=(($d*$b)+$e)-$mz;
    $efectivo=($total-($a * $b));
    DB::table('nominas_movimientos')->select('nominas_movimientos')
    ->where('nominas_movimientos.nomina_id','=', $request->nomina_id)
    ->where('nominas_movimientos.contrato_id','=',$request->contrato_id)
    ->where('nominas_movimientos.empleado_id','=',$request->empleado)->delete();
    $mov_nomi = new \App\MovimientosNomina();
    ...
    return response()->json($mov_nominas);
  }
  ...
  public function show($id)
  {
    ...
    ->where('nominas_movimientos.empleado_id','=',$valores[0])
    ->where('nominas_movimientos.contrato_id','=',$valores[1])
    ->where('nominas_movimientos.nomina_id','=',$valores[2])
    ->get();
  ...
  {
    $resultado = DB::select("SELECT semana FROM  sueldo_semana_empleado_registros GROUP BY semana");

    return response()->json($resultado);
    // code...
  }

  public function getSemanasEmpleado($id)
  {

    $semana = DB::select("SELECT semana,sueldo_semana_empleado_id,e.nombre as nombre FROM sueldo_semana_empleado_registros as sr
      JOIN sueldo_semana_empleado as e ON e.id = sr.sueldo_semana_empleado_id
      WHERE sr.semana = '$id'
      AND sr.total != 0
      GROUP BY sr.sueldo_semana_empleado_id");

      $arreglo = [];
      foreach ($semana as $key => $value) {
        $totales = DB::table('sueldo_semana_empleado_registros')
        ->select(DB::raw("(SUM(total)) AS total"))
        ->where('sueldo_semana_empleado_id',$value->sueldo_semana_empleado_id)
        ->where('semana',$id)->first();

        $totales_resta = DB::table('sueldo_semana_empleado_registros')
        ->select("total")
        ->where('sueldo_semana_empleado_id',$value->sueldo_semana_empleado_id)
        ->where('semana',$id)->first();
    ...
    public function saveNB(Request $request)
    {
      try {
        ...
        if ($nm->empresa == 3) {//CSCT
          ...
          $nm->banco_id = $dbe->id;
          $nm->save();
        }elseif ($nm->empresa == 1) {//CONSERFLOW
