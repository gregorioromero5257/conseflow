        ...
        <div class="card-body">
          ...
          <div class="form-row">
            <div class="form-group col-md-4">
              ...
              <input type="text" v-model="data.dias_t_dias" class="form-control" name="" v-validate="'required'" value="" data-vv-name="Dias">
              <span class="text-danger">{{errors.first('Dias')}}</span>
            </div>
            <div class="form-group col-md-4">
              <label>Total de meses</label>
              <input type="text" v-model="data.dias_t_meses" class="form-control" name="" v-validate="'required'" value="" data-vv-name="Meses">
              <span class="text-danger">{{errors.first('Meses')}}</span>
            </div>
            <div class="form-group col-md-4">
              <label>Total de años</label>
              <input type="text" v-model="data.dias_t_anio" class="form-control" name="" v-validate="'required'" value="" data-vv-name="Años">
              <span class="text-danger">{{errors.first('Años')}}</span>
            </div>
          </div>
          <div class="form-row">
          ...
          <!-- <div class="form-row">
            <div class="form-group col-md-4">
              <span class="text"><h6>Sueldo diario del trabajador </h6></span>
            </div>
            <div class="form-group col-md-4">
              <span class="text"><h6>$&nbsp;{{((parseFloat(data.final) + parseFloat(data.couta_patronal))/30.4).toFixed(2)}}</h6></span>
            </div>
          </div>
            <hr> -->
          <div class="form-row">
            ...
            <div class="form-row">
              ...
              <!-- <div class="form-row">
                <div class="form-group col-md-4">
                  <span class="text"><h6>Sueldo total periodo&nbsp;</h6></span>
                </div>
                <div class="form-group col-md-4">
                  <span class="text"><h6>$&nbsp;{{(((parseFloat(data.final) + parseFloat(data.couta_patronal))/30.4) * data.dias_t).toFixed(2)}}</h6></span>
                </div>
              </div>
                <hr> -->
                <div class="form-row">
                  <div class="form-group col-md-4">
                    <span class="text"><h6>Aguinaldo&nbsp;</h6></span>
                  </div>
                  <div class="form-group col-md-4">
                    <span class="text"><h6>$&nbsp;{{(data.aguinaldo).toFixed(2)}}</h6></span>
                  </div>
                </div>
                  <hr>
                  <div class="form-row">
                    <div class="form-group col-md-4">
                      <span class="text"><h6>Prima Vacacional&nbsp;</h6></span>
                    </div>
                    <div class="form-group col-md-4">
                      <span class="text"><h6>$&nbsp;{{(data.prima_vacacional).toFixed(2)}}</h6></span>
                    </div>
                  </div>
                    <hr>
                    <div class="form-row">
                      <div class="form-group col-md-4">
                        <span class="text"><h6>Vacaciones&nbsp;</h6></span>
                      </div>
                      <div class="form-group col-md-4">
                        <span class="text"><h6>$&nbsp;{{(data.vacaciones).toFixed(2)}}</h6></span>
                      </div>
                    </div>
                      <hr>
              <div class="form-row">
                ...
                <!-- <div class="form-row">
                  <div class="form-group col-md-4">
                    <span class="text"><h6>Costo total del periodo&nbsp;</h6></span>
                  </div>
                  <div class="form-group col-md-4">
                    <span class="text"><h6>$&nbsp;{{((((parseFloat(data.final) + parseFloat(data.couta_patronal))/30.4) * parseFloat(data.dias_t)) + parseFloat(data.finiquito)).toFixed(2)}}</h6></span>
                  </div>
                </div>
                  <hr> -->
            <!-- <div class="form-row">
              <div class="form-group col-md-4">
                <span class="text"><h6>Costo diario trabajador&nbsp; </h6></span>
              </div>
              <div class="form-group col-md-4">
                <span class="text"><h6>$&nbsp;{{(((((parseFloat(data.final) + parseFloat(data.couta_patronal))/parseFloat(data.periodo_num)) * parseFloat(data.dias_t)) + parseFloat(data.finiquito)) / parseFloat(data.dias_t)).toFixed(2)}}</h6></span> -->
...
export default {
  data (){
    return {
      ...
      aniosLab : [
        0.00,1.00,2.00,3.00,4.00,5.00,10.00,15.00,20.00,25.00,30.00
      ],
      isLoading: false,
    }
  },
  ...
  methods : {
    cambio(){
      if(this.opciones.periodo == ''){
        toastr.warning('Seleccione Periodo');
      }else {
        if (this.opciones.periodo === 'Semanal') {
          this.data.inicial = (7 * this.data.sueldo_bruto_d).toString();
          this.data.periodo_num = 7;
        }else if (this.opciones.periodo === 'Quincenal') {
          this.data.inicial = (15 * this.data.sueldo_bruto_d).toString();
          this.data.periodo_num = 15;
        }else if (this.opciones.periodo === 'Mensual') {
          this.data.inicial = (30.4 * this.data.sueldo_bruto_d).toString();
          this.data.periodo_num = 30.4;
        }
      }
    },
    ...
    factorintegracion(){
      var uno,dos,tres,dos_uno;
      uno = 1;
      if(parseFloat(this.data.dias_t) <= 365)
      {
        dos_uno = 15;
      }else {
        dos_uno = (parseFloat(this.data.dias_t) / 365) * 15
      }
      dos = dos_uno / 365;
      tres = (this.data.dias_vacaciones * 0.25) / 365;
      //console.log(uno+' '+dos+' '+tres);
    return this.data.factor = (uno + dos + tres) ;

    },

    calcSBC(value) {
      var sbc;
      // console.log(value,'dddddllsdf');
...
// 11.63
finiquito(){
  var dias = this.calculo_Dias();
  var vac = this.DiasVac(dias);

  var finiquito;
  var factor_dia_trabajado = 15 / dias;
  var uno;
  uno = factor_dia_trabajado * parseFloat(this.data.dias_t);
  this.data.aguinaldo = uno * this.data.sdi;
  // console.log(uno,'--',factor_dia_trabajado);

  var factor_dia_trabajado_vac = vac / 365;
  var dos;
...
calculo_Dias(){
    var dia = this.data.dias_t_dias;
    var dia_meses = Math.trunc(this.data.dias_t_meses * 30.4);
    var dia_anios = Math.trunc(this.data.dias_t_anio * 365);
    var diaf = parseInt(dia) + parseInt(dia_meses) + parseInt(dia_anios);
  // console.log(dia,'idid');
  this.data.dias_t = diaf;
  return this.data.dias_t;
},

DiasVac(d){
  var tabla_uno , tabla_dos,position,i;
   this.aniosLab;
  for (i=0; i < this.aniosLab.length; i++) {
    if ((parseFloat(d)/365) >= this.aniosLab[i] && (parseFloat(d)/365) < this.aniosLab[i+1]) {
      position = i;
    }
    else if ((parseFloat(d)/365) >= this.aniosLab[this.aniosLab.length - 1]) {
      position = this.aniosLab.length - 1;
    }
  }
  tabla_dos = this.diasVac[position];
  this.data.dias_vacaciones = tabla_dos;
  ...
  return tabla_dos;
    ...
    resetValues () {
      ...
      this.data.dias_t = '';
      this.data.dias_t_anio = 0;
      this.data.dias_t_dias = 0;
      this.data.dias_t_meses = 0;
