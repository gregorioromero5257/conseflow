    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                ...
                <button @click="abrirModal(true)" class="btn btn-dark float-sm-right">
            ...
            <div class="modal-dialog modal-dark modal-lg" role="document">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-secondary" @click="Guardar(true)"><i class="fas fa-save"></i>&nbsp;Guardar
                        </button>
                        <!-- <button type="button" class="btn btn-secondary" @click="Guardar(false)"><i class="fas fa-save"></i>&nbsp;Actualizar
                        </button> -->
                    </div>
...
export default
{
    data()
    {
        return {
            ...
            listaEmpleados: [],
            listaRegistros: [],
            columns: ['id', 'empleado', 'fecha', 'horas', 'proyecto'],
            options:
            {
                headings:
                {
    ...
    methods:
    {
        ...
        Guardar(nuevo)
        {
            ...
            console.error(this.controlt);

            let data_dts = "";
            this.listaRegistros.forEach(e =>
            {
                let data = e.empleado_id + "&" + e.proyecto_id + "&" + e.fecha + "&" + e.horas;
                data_dts += data + "|";
            });
            // console.error(data_dts);
            if (!nuevo)
                data.append("id", this.controlt.id);
            data.append("datos", data_dts);
        ...
        ObtenerRegistros()
        {
            axios.get(this.url + "/obtener").then(res =>
            {
                if (res.data.status)
                {
                    this.tableData = res.data.controles;
                }
                else
                {
                    toastr.error(res.data.mensaje);
