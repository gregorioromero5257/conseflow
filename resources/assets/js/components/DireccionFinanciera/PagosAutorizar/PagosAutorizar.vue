          ...
          <template v-if="registros != null ">
            <div class="form-row">
                  ...
                  <div class="card-body" >
                    <paginate name="uno" :list="registros[0]" :per="3">
                      <div v-for="lang in paginated('uno')">
                        ...
                        <template v-if="lang.condicion_pnr == 4 || lang.condicion_pnr == 7">
              ...
              <div class="card">
                ...
                <div class="card-body">
                  <paginate name="facturas_proveedores" :list="list_facturas_proveedores" :per="6">
                    <div v-for="factura in paginated('facturas_proveedores')" class="col-auto">
                      <div class="row">
                        <div class="col-1">
                          <button class="btn btn-sm rounded-circle border-dark btn-secodary border" type="button" 
                            data-toggle="collapse" :data-target="`#collapse_${factura.fp_id}`" aria-expanded="false" 
                            :aria-controls="`#collapse_${factura.fp_id}`">
                        ...
                        <div class="col-auto" style="margin-left:-1rem;">
                          ...
                          <span class="badge badge-pill badge-info">
                            {{factura.fecha_programada}}
                          </span>
                          </h6>
                        </div>
                      </div>
                        
                      <div class="collapse" :id="`collapse_${factura.fp_id}`">
                        <div class="card-body">
                              <table>
                                <tr>
                                  <th scope="col">Proveedor</th>
                                  <th scope="col" style="padding-left:3rem;">Urgente</th>
                                </tr>
                                <tr>
                                  <td>{{factura.proveedor}}</td>
                                  <td style="padding-left:3rem;">{{factura.prioridad == '1' ? 'SÍ' : 'NO'}}</td>
                                </tr>
                                <hr>
                                <tr>
                                  <th scope="col">Condición Pago</th>
                                  <th scope="col" style="padding-left:3rem;">Periodo Entrega</th>
                                </tr>
                                <tr>
                                  <td>{{factura.condicion_pago_id == 1 ? 'Crédito' : 'Contado'}}</td>
                                  <td style="padding-left:3rem;">{{factura.periodo_entrega}}</td>
                                </tr>
                                <hr>
                                <tr>
                                  <th scope="col">Moneda</th>
                                  <th style="padding-left:3rem;" scope="col">Monto</th>
                                </tr>
                                <tr>
                                  <td>{{factura.moneda == 2 ? 'MXN' : 'USD'}}</td>
                                  <td style="padding-left:3rem;">{{factura.total}}</td>
                                </tr>
                                <hr>
                                <tr>
                                  <th scope="col">Empleado Elabora</th>
                                  <th style="padding-left:3rem;" scope="col">Empleado Autoriza</th>
                                </tr>
                                <tr>
                                  <td>{{factura.nombre_empleado_elabora}}</td>
                                  <td style="padding-left:3rem;">{{factura.nombre_empleado_autoriza}}</td>
                                </tr>
                                <hr>
                              </table>
                              <div class="row ml-1">
                                  <button @click="AutrizarProveedor(factura.fp_id)" type="button" class="btn btn-success">Autorizar</button>
                              </div>
                        </div>
                      </div>
                  ...
                  <paginate-links for="facturas_proveedores"
                    :simple="{
                      prev: 'Anterior',
                      next: 'Siguiente'}"
                    :classes="{
        ...
        export default{
          data () {
            return {
              ...
              registros: null,
              vencidos : null,
              paginate: ['uno','dos','tres','cuatro','facturas_proveedores'],
              fecha_now : '',
              detalle_uno : false,
              data_uno : null,
              detalle_dos : false,
          ...
          methods : {
              ...
              axios.get('pagosporautorizar').then(response => {
                ...
                this.meses = response.data.mhy;
                  response.data.a[0].forEach((item, i) => {
                    ...
                    if (item.condicion_pnr == 4 || item.condicion_pnr == 7) {
            ...
            {
              axios.get("/conta/facturasproveedores/obtenerprogramados/=&4")
              .then(res=>
              {
                if(res.data.status)
                {
                  this.list_facturas_proveedores=res.data.pagos;
                }
                else{
                  toastr.error("Error al obtener las facturas");
                console.error(r);
                }
              }).catch(r=>
              {
            ...
            AutrizarProveedor(id)
            {
              Swal.fire({
                title: '¿Autorizar el pago de la factura?',
                showDenyButton: true,
                ...
                if (result.value) {
                  axios.put("/conta/facturasproveedores/autorizar",{id}).then(res=>
                  {
                    if(res.data.status)
                    {
                      $("#collapse_"+id).removeClass("show")
                      toastr.success("Pago autorizado correctamente");
                      //this.list_facturas_proveedores=res.data.pagos;
                      this.getFacturasProveedores();
                    }
                    else{
