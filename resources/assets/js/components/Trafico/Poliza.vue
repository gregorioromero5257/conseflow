...
export default {
  data (){
    return{
      ...
      id : '',
      datapoliza: [],
      columnspoliza: ['id','proveedor','numero_poliza','numero_inciso','fecha_inicio','fecha_termino'],
      isLoading : false,
      poliza : {
        id : 0,
        proveedor : '',
      ...
      optionspoliza: {
        ...
        sortable: ['proveedor','numero_poliza','numero_inciso','fecha_inicio','fecha_termino' ],
        filterable: [ 'proveedor','numero_poliza','numero_inciso','fecha_inicio','fecha_termino'],
        filterByColumn: true,
        listColumns: { },
        texts:config.texts
      },
  ...
  methods :{
    ...
    getData(){
      this.isLoading = true;
      axios.get('polizasunidades/'+ this.id).then(response => {
    ...
    guardarMto(nuevo){
      this.$validator.validate().then(result =>{
        if (result) {
          ...
          axios.post('polizasunidades',formData).then(response => {
            me.isLoading = false;
            if (response.data.status) {
              ...
              {
                toastr.success('Conductor Registrado Correctamente');
                me.getData();
                me.cerrarModal();
              }
            }
    ...
    descargar(archivo){
      let me=this;
      axios({
        url: '/polizasunidadesdescarga/' + archivo,
        method: 'GET',
