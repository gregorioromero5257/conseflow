<template>
  <main class="main">
    <div class="container-fluid">
      <!-- Ejemplo de tabla Listado -->{{descal}}
      <br>
      <div class="card">
            ...
            <template slot="id" slot-scope="props">
              ...
              <div class="btn-group dropup" role="group">
                ...
                <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                  ...
                  <button v-show="PermisosCRUD.Update" type="button" @click="abrirModal('articulo','actualizar',props.row)" class="dropdown-item" >
              ...
              <template v-if="props.row.condicion">
                <button v-show="PermisosCRUD.Delete" type="button" class="dropdown-item" @click="desactivarArticulo(props.row.id)" >
                  <i class="fas fa-ban"></i>&nbsp;Desactivar Articulo
                </button>
              </template>
              <template v-else>
                <button type="button" class="dropdown-item" @click="activarArticulo(props.row.id)" >
                  <i class="icon-check"></i>&nbsp;Activar Articulo
                </button>
              </template>

                  <button  type="button" class="dropdown-item" @click.prevent="mostrarExistencias(props.row)" >
                    <i class="fas fa-boxes"></i>&nbsp;Existencias de Almacen</button>

                  <button v-show="PermisosCRUD.Read" type="button" class="dropdown-item" @click.prevent="mostrarKardex(props.row)" >
                    <i class="fas fa-people-carry"></i>&nbsp;Movimientos</button>

                    <button v-show="PermisosCRUD.Read" type="button" class="dropdown-item" @click.prevent="verprecioventa(props.row)" >
                      <i class="fas fa-people-carry"></i>&nbsp;Precio de venta</button>

                  <button  type="button" class="dropdown-item"  v-if="props.row.fotografia != null" @click="descargar(props.row.fotografia)" >
                    <i class="fas fa-boxes"></i>&nbsp;Descargar Imagen de Articulo</button>


                  <button  type="button" class="dropdown-item" v-if="props.row.ficha_tecnica != null" @click="descargar(props.row.ficha_tecnica)" >
                    <i class="fas fa-boxes"></i>&nbsp;Descargar Ficha Tecnica</button>
                </div>
              </div>
              </div>
            </template>
                ...
                <div class="row">
                  ...
                  <div class="col">
                    ...
                    <div class="form-group row" >
                      ...
                      <div class="form-group row" v-if="false" >
                        <label class="col-md-3 form-control-label">Código de producto:</label>
                        <div class="col-md-5">
                            <input type="text" class="form-control" v-model="txtProductoSat">
                        </div>
                        <div class="col-md-2">
                            <button type="button" class="btn btn-outline-dark" @click="abrirModalSat()">
                              <i class="fas fa-edit"></i>&nbsp;Ingresar
                            </button>
                        </div>
                      </div>
...
export default {
  ...
  data (){
    return {
      ...
      modalKardex: 0,
      modalPrecio : 0,
      columns: ['id', 'nombre', 'codigo','trnom','descripcion', 'marca', 'unidad', 'comentarios', 'minimo', 'maximo', 'grupo', 'categoria','calidad', 'condicion'],
      tableData: [],
      options: {
        headings: {
          nombre: 'Nombre',
...
methods : {
  ...
  RegistrarActualizar(nuevo){
    ...
    this.$validator.validate().then(result => {
      if (result) {
        ...
        // formData.append('codigoProductoSat', this.codigosSat_id);
      ...
      case "articulo":
      {
        switch(accion){
          ...
          case 'actualizar':
          {
            ...
            axios.get('/categoria/getlist').then(response => {
              me.listaCategorias = response.data;
              axios.get('/grupo/getlist/' + this.categoria_id).then(response => {
                me.listaGrupos = response.data;
  ...
  verprecioventa(data){
    ...
    axios.get('/precioventa/' + data.id).then(response => {
      swal(
        ...
        'Precio de venta!',
        'El precio de venta por unidad es: $ '+  (parseFloat(new Intl.NumberFormat("en-US").format(response.data.toFixed(2))))
        // 'success'
      )
    }).catch(error => {
      console.error(error);
  ...
  abrirModalSat()
  {
    ...
    {
      me.listaTiposSsat=res.data.tipos_sat;
    })
    .catch(function (error)
    {
      console.log(error);
  ...
  buscarCodigoSat()
  {
    let me=this;
    me.loading=true;
    axios.get('/codigosat/10101500').then( res=>
    {
  ...
  {
    let me =this;
    if(me.codigosSat_id==0)
    {
      toastr.success('Error');
    }
    else
    {
      axios.get('codigosat/getproducto/'+me.codigosSat_id).then( res=>
      {
        let producto=res.data.producto;
        me.txtProductoSat=producto.codigo + " - " + producto.nombre;
        // console.log(producto);
        toastr.success('Código seleccionado');
      }).catch( error=>
      {
        console.error(error);
      })
