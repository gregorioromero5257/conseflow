...
export default
{
    ...
    methods:
    {
        // Obtiene todas las rimes del proyecto ingresado
        ObtenerRimes()
        {
            this.isRimesLoading = true;
            axios.get("/calidad/resumen/rime/" + this.proyecto_id).then(res =>
            {
                this.isRimesLoading = false;
                if (res.data.status)
                {
                    this.list_rimes = res.data.rimes;
                }
                else
                {
        ...
        {
            window.open("/calidad/inspecciones/descargar-certificado/" + rime_id, "_blank");
        },
        // Descarga el formato de la rime seleccionada
        DescargarRime(rime_id)
        {
            window.open("/calidad/inspecciones/descargar/" + rime_id, "_blank");
        }
