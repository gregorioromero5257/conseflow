          ...
          <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
            <div class="btn-group dropup" role="group">
              ...
              <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                <button type="button" v-show="props.row.cantidad_entrada == props.row.cantidad" class="dropdown-item" @click="eliminarpartidacompra(props.row,0)">
                  <i class="icon-trash"></i>&nbsp;Eliminar.
                </button>
                <button type="button" v-show="props.row.cantidad_entrada == props.row.cantidad" class="dropdown-item" @click="actualizarpartidacomprauno(props.row, 0)">
        ...
        <form>
          ...
          <div class="form-row" >
            <div class="form-group col-md-8">
              ...
              <div class="input-group">
                <input type="text"  v-model="partidascompras.nombrearti" v-bind:class="'form-control'+clases.nombrearti" placeholder="Articulo" readonly>
                <div class="input-group-append">
                  <button class="btn btn-secondary" type="button" v-bind:disabled ="desabilitar" @click="abrirModalA('articulo','registrar',partidascompras.orden_compra_id)">Buscar</button>
          ...
          <div class="form-row">
            <div class="form-group col-md-8">
              ...
              <textarea class="form-control"  v-model="partidascompras.comentario" autocomplete="off" id="comentario" rows="3"></textarea>
            </div>
          </div>
...
export default {
  data() {
    return {
      ...
      columnscompras: ['id','ad','cantidad','precio_unitario','total'],//
      optionsart: {
        headings: {
          nombre: 'Nombre',
...
methods: {
  ...
  cargarpartidas(data){
   ...
   (data.condicion == 2) ? this.desabilitar = true : this.desabilitar = false;
    ...
    let me= this;
    axios.get(`compras/gastos/${data.id}`).then(response => {
  ...
  guardarPR(nuevo, idr){
    if (this.validado == 1) {
      ...
      axios({
       ...
       url: nuevo ? `partida/compra/gasto/create` : '/requisicioncomprahdh'+'/'+this.partidascompras.requisicione_id,
        data: {
  ...
  */
  eliminarpartidacompra(data,activar){
    ...
    swal({
      ...
      if (result.value) {
        let me = this;
        axios.get(`partida/orden/compra/gasto/eliminar/${data.id}`).then(function (response) {
          if(!activar){
