      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <v-client-table :data="tableData" :options="options" :columns="columns">
            <template slot="folio_requisicion" slot-scope="props">
              {{String(props.row.folio_requisicion).replace('REQ-','')}}
            </template>
            <template slot="descripcion_oc" slot-scope="props">

              <template v-if="props.row.folio_orden_compra != null">
                {{props.row.descripcion}}<br>{{props.row.poc_comentario}}
              </template>
            </template>
            <template slot="folio_orden_compra" slot-scope="props" >
              <template v-if="props.row.folio_orden_compra != null">
                {{String(props.row.folio_orden_compra).replace('OC-CONSERFLOW-020-','')}}
              </template>
            </template>
          </v-client-table>
        </div>
...
export default {
  data(){
    return{
      ...
      proyecto_id : '',
      columns: [
          'nombre','folio_requisicion','descripcion','cantidad','partida_orden_compra_cantidad','folio_orden_compra','descripcion_oc','fecha_orden','periodo_entrega'
      ],
      tableData: [],
      options: {
          headings: {
