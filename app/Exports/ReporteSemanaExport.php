...
class ReporteSemanaExport implements FromView, WithEvents
{
  protected $id;
  ...
  public function view(): View
  {
    ...
    for ($i=5; $i < 42 ; $i++) {
      ...
      $fecha_inicio_semana = '2020-01-27';
      ...
      foreach ($arreglo_nom as $k_m => $mov) {
        //lo que registra RH
        $semana_n = SemanaN::where('semana',$i)
        ...
        if (isset($nomina_c) == true) {
          ...
          $suma_d = $sumad->sd;
          $ot = OtroPago::where('factura_nomina_id',$nomina_c->factura_nomina_id)->get();
        ...
        if (isset($semana_n) == true || isset($nomina_c) == true) {
          if (isset($semana_n) == true && (float)$semana_n->dl != 0) {
            ...
            'factura' => isset($nomina_c) == true ? $nomina_c : $this->calcular((float)$semana_n->dl,"Semanal",$pago_ordinario),
            'de' => $d,
            'pe' => $p,
            'otros' => $ot,
            'salario' => $salario,
            'po' =>$pago_ordinario,
            'sa' => $mov->salario,
            'netoc' =>  $this->calcular((float)$semana_n->dl,"Semanal",$pago_ordinario),
            'infonavit' => $i_final,
            'suma_d' => $suma_d,
            ];
          }
      ...
      $arreglo_masivo [] =  $arreglo_f;

    }

    return view('excel.reportesemanal', compact('a','arreglo_masivo'));
  }
